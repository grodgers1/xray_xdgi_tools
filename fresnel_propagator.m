function [ I_z, WF_z ] = fresnel_propagator( WF, pixsize, z, lambda, method )
...
sx = length(WF); % size of input wavefunction
x = ((0:sx-1)-(sx-1)/2)*pixsize; % position vector
norm_factor = mean(mean(real(WF))); % for normalizing intensity of carpet
I_z = zeros(length(z),length(WF)); % initialize Talbot carpet (intensity profile)
WF_z = zeros(length(z),length(WF)); % initialize wavefront at position z

% Method 1: real space propagator, convoluted by fft then multiplication
% with fft of wavefunction
fft_WF = fftshift(fft(WF)); % fourier transfer for convolution
if method == 1
    for ii = 1:length(z)
